...
class ImageGenerationController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $model = trim((string) config('services.gemini.image_model', ''));
        ...
        try {
            ...
            for ($index = 0; $index < 1; $index += 1) {
                $variantPrompt = $this->buildVariantPrompt($prompt, $controls, $index + 1);
                if (is_string($activeModel) && $activeModel !== '') {
                    $gemini = $this->requestGeminiImage($activeModel, $apiKey, $variantPrompt);
                } else {
                    $result = $this->requestGeminiImageWithFallback($model, $apiKey, $variantPrompt);
                    $activeModel = $result['model'];
                    $gemini = $result['payload'];
                }

                if (!$revisedPrompt && !empty($gemini['text'])) {
                    $revisedPrompt = $gemini['text'];
                }

                $images[] = $this->mapImagePayload(
                    imageData: $gemini['imageData'],
                    mimeType: $gemini['mimeType'],
                    prompt: $prompt,
                    revisedPrompt: $revisedPrompt ?? $variantPrompt,
                    aspectRatio: (string) $controls['aspectRatio'],
                    stylePreset: (string) $controls['stylePreset'],
                    index: $index + 1,
                );
            }
            ...
            return response()->json([
                'images' => $images,
                'revisedPrompt' => $revisedPrompt ?? $this->buildRevisedPrompt($prompt, $controls),
    ...
    private function requestGeminiImage(string $model, string $apiKey, string $prompt): array
    {
        ...
        foreach ($attempts as $responseModalities) {
            ...
            $parsed = $this->parseGenerateContentResponse($model, $response->json());
    ...
    private function parseGenerateContentResponse(string $model, array $body): ?array
    {
    ...
    private function mapImagePayload(
        ...
        return [
            'id' => 'img-'.Str::uuid()->toString(),
            'label' => 'Variant '.$index,
            'url' => "data:{$mimeType};base64,{$imageData}",
            'alt' => "{$stylePreset} concept for {$prompt}",
            'width' => $size['width'],
            'height' => $size['height'],
            'prompt' => $prompt,
    ...
    private function buildVariantPrompt(string $prompt, array $controls, int $variant): string
    {
        return trim(implode("\n", [
            $prompt,
            "Style preset: {$controls['stylePreset']}",
            "Aspect ratio: {$controls['aspectRatio']}",
    ...
    {
        return trim("{$prompt}, {$controls['stylePreset']} style, {$controls['aspectRatio']} composition, {$controls['quality']} quality");
    }
